...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 1 ; i <= N; i++){
            if(parent[i] < 0) { // 음수이면 선분 그룹을 대표하는 부모 노드이므로 카운트
                ans++;
                res = Math.max(res, -parent[i]); // 음수의 절댓값이 선분 그룹의 선분 개수
            }
        }
    ...
    private static int find(int i){
        if(parent[i] < 0) // 음수이면 자기 자신이 부모노드
            return i;
    ...
    private static void union(int i, int j){
        ...
        if(p == q) // 이미 연결되어있는 경우
            return;
        // 더 큰 그룹에 작은 그룹을 합치기 (Union by Size)
